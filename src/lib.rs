...
mod usage {
    ...
    fn easy_nat_example() {
        ...
        let mut nat = NATRouter::<EASY_NAT, 1>::new([nat_ex_addr], 10000..=19999, 49152..=u16::MAX, 12, timeout);
        ...
        time += 100;
        let reroute = nat.route_external_packet(server_ex_addr, server_ex_port, nat_ex_addr, client_in_port, false, time);
    ...
    fn full_cone_example() {
        ...
        let nat_ex_addr = 11111;
        let mut nat = NATRouter::<FULL_CONE_NAT, 1>::new([nat_ex_addr], 100000..=200000, 49152..=u16::MAX, 12, timeout);
        ...
        match nat.route_internal_packet(client_in_addr, client_in_port, server_ex_addr, server_ex_port, time) {
            ...
            DestType::External { external_src_addr, external_src_port } => {
                ...
                time += 100;
                let reroute = nat.route_external_packet(server_ex_addr, server_ex_port, external_src_addr, external_src_port, false, time);
    ...
    fn symmetric_nat_example() {
        ...
        let nat_ex_addr = 11111;
        let mut nat = NATRouter::<SYMMETRIC_NAT, 1>::new([nat_ex_addr], 100000..=200000, 49152..=u16::MAX, 12, timeout);
        ...
        time += 100;
        let reroute0 = nat.route_internal_packet(client_in_addr, client_in_port, server_ex_addr, server_ex_port0, time);
        let reroute1 = nat.route_internal_packet(client_in_addr, client_in_port, server_ex_addr, server_ex_port1, time);
        match (reroute0, reroute1) {
            (External { external_src_addr: ex_src_addr0, external_src_port: ex_src_port0 }, External { external_src_addr: ex_src_addr1, external_src_port: ex_src_port1 }) => {
                ...
                time += 100;
                let reroute = nat.route_external_packet(server_ex_addr, server_ex_port0, ex_src_addr1, ex_src_port1, false, time);
                assert!(reroute.is_none());
            }
            _ => assert!(false),
        }
    ...
    fn hard_nat_example() {
        ...
        let mut nat = NATRouter::<HARD_NAT, 4>::new([11110, 11111, 11112, 11113], 100000..=200000, 49152..=u16::MAX, 245, timeout);
        ...
        match (reroute0, reroute1) {
            (External { external_src_addr: ex_src_addr0, external_src_port: ex_src_port0 }, External { external_src_addr: ex_src_addr1, external_src_port: ex_src_port1 }) => {
                ...
                // before the timeout, but this hard NAT only refreshes the timeout when the client
                // sends a packet.
                time += timeout - 1;
                let reroute = nat.route_external_packet(server_ex_addr, server_ex_port0, ex_src_addr0, ex_src_port0, false, time);
                assert!(reroute.is_none());
            }
            _ => assert!(false),
        }
    ...
    fn restricted_firewall() {
        ...
        time += 100;
        let reroute = firewall.route_internal_packet(client_addr, client_port, server0_addr, server_port, time);
    ...
    fn port_restricted_firewall() {
        ...
        time += 100;
        let reroute = firewall.route_internal_packet(client_addr, client_port, server_addr, server0_port, time);
        assert!(reroute.is_external());

        time += 100;
        let reroute = firewall.route_external_packet(server_addr, server1_port, client_addr, client_port, false, time);
        assert!(reroute.is_none());
    }
}
